...
Description:
    프로젝트 환경변수를 불러오고 외부 API 인증키를 관리한다.
"""
...
def load_api_key(env_file: str = DEFAULT_ENV_FILE) -> str:
    ...
    """
    return _load_env_value(SEOUL_API_KEY_NAME, env_file)


def load_openai_api_key(env_file: str = DEFAULT_ENV_FILE) -> str:
    """OpenAI API 인증키를 환경변수 또는 .env에서 불러온다."""
    return _load_env_value(OPENAI_API_KEY_NAME, env_file)


def _load_env_value(name: str, env_file: str) -> str:
    """Load one required secret, preferring the process environment."""
    api_key = os.getenv(name, "").strip()

    if not api_key and os.path.isfile(env_file):
        try:
            api_key = str(dotenv_values(env_file).get(name, "")).strip()
        except (OSError, UnicodeError) as error:
